<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:000000,100:00FF41&height=200&section=header&text=Kapil%20Kumbhare&fontSize=50&fontColor=00FF41&fontAlignY=38&desc=Linux%20%7C%20Cloud%20%7C%20DevOps&descAlignY=58&descSize=20&descColor=ffffff" width="100%"/>

### 🟢 Open to Work — Entry-Level DevOps / Cloud / Linux Roles

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Fira+Code&size=17&pause=1000&color=00FF41&center=true&vCenter=true&width=650&lines=Building+Self-Healing+Linux+Infrastructure;AWS+Cloud+%26+DevOps+Explorer;Automating+Everything+with+Bash;Fresher+who+builds+real+systems%2C+not+tutorials)](https://git.io/typing-svg)

</div>

---

<img align="right" alt="DevOps Engineering" width="260" src="https://user-images.githubusercontent.com/74038190/229223263-cf2e4b07-2615-4f87-9c38-e37600f8381a.gif"/>

## `$ whoami`

```bash
Name     : Kapil Kumbhare
Handle   : @kapilskumbhare-cyber
Role     : Aspiring DevOps & Cloud Engineer
Location : India 🇮🇳
Status   : ✅ Actively seeking entry-level roles & internships
Contact  : kapilskumbhare@gmail.com
Mindset  : Build first. Learn deeper. Repeat.
```

I'm a fresher who doesn't just study tools — I **build real systems** with them.

My projects tackle actual sysadmin problems: automated failure detection, self-healing services, secure user management, and process control. Every repo on my profile is hands-on, not tutorial-copy.

> *"A system that heals itself is better than an engineer who stays up all night."*

<br clear="right"/>

---

## 🛠️ Tech Stack

**Operating Systems & Shell**

![Linux](https://img.shields.io/badge/Linux-FCC624?style=for-the-badge&logo=linux&logoColor=black)
![Ubuntu](https://img.shields.io/badge/Ubuntu-E95420?style=for-the-badge&logo=ubuntu&logoColor=white)
![Bash](https://img.shields.io/badge/Bash-4EAA25?style=for-the-badge&logo=gnu-bash&logoColor=white)

**Cloud & Version Control**

![AWS](https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazon-aws&logoColor=FF9900)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)

**Scripting & Automation**

![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![Shell Script](https://img.shields.io/badge/Shell_Script-121011?style=for-the-badge&logo=gnu-bash&logoColor=white)

---

## 🚀 Featured Project

### 🛡️ [linux-autoheal-monitor](https://github.com/kapilskumbhare-cyber/linux-autoheal-monitor)

> **Modular Linux Monitoring System with Alerting, Decision Engine & Automated Self-Healing using Bash**

This is my **capstone Linux project** — not a script, but a full system built with production thinking.

Most beginners write a script that prints CPU usage. I built a system that **detects the problem, decides what to do, alerts the team, and fixes itself** — automatically.

<details>
<summary><b>📐 Click to expand System Architecture</b></summary>

```
linux-autoheal-monitor/
│
├── modules/          → Individual monitors
│   ├── cpu.sh        → CPU usage tracking
│   ├── memory.sh     → RAM usage tracking
│   ├── disk.sh       → Disk space monitoring
│   ├── service.sh    → Critical service checks
│   └── security.sh   → Security event monitoring
│
├── engine/           → Decision logic
│   └── decision.sh   → Analyzes data, detects anomalies
│
├── alert/            → Notification system
│   └── alert.sh      → Sends email alerts on threshold breach
│
├── healing/          → Auto-recovery
│   └── heal.sh       → Restarts failed services automatically
│
├── logs/             → Persistent audit logs
├── config.conf       → Configurable thresholds
└── monitor.sh        → Entry point → chmod +x && ./monitor.sh
```

</details>

| Feature | Details |
|---|---|
| 📊 Real-time Monitoring | CPU · RAM · Disk · Services · Security events |
| 🧠 Decision Engine | Analyzes system state, detects threshold breaches |
| 🚨 Alerting | Automated email notifications via Mail Utility |
| 🔁 Self-Healing | Auto-restarts failed services — zero manual intervention |
| 📝 Logging | Full audit trail for post-incident analysis |

**Stack:** `Linux` `Bash` `Cron Jobs` `Systemctl` `Mail Utility` `Git`

---

## 🗂️ Linux Engineering Projects

> Hands-on Linux builds covering the core skills every DevOps engineer uses daily.

<table>
  <tr>
    <td width="33%">
      <h3>🔐 <a href="https://github.com/kapilskumbhare-cyber/linux-file-permissions">linux-file-permissions</a></h3>
      <p>Implemented file permission fundamentals, secure multi-user directory setup, and access control from scratch.</p>
      <code>chmod</code> <code>chown</code> <code>ACLs</code> <code>umask</code>
    </td>
    <td width="33%">
      <h3>👥 <a href="https://github.com/kapilskumbhare-cyber/linux-user-management">linux-user-management</a></h3>
      <p>Built a full user onboarding flow with group-based access control, password policies, and secure system administration.</p>
      <code>useradd</code> <code>groupadd</code> <code>passwd</code> <code>sudo</code>
    </td>
    <td width="33%">
      <h3>⚙️ <a href="https://github.com/kapilskumbhare-cyber/linux-process-management">linux-process-management</a></h3>
      <p>Explored the complete Linux process lifecycle — monitoring, controlling, and managing running processes.</p>
      <code>ps</code> <code>top</code> <code>kill</code> <code>jobs</code> <code>systemctl</code>
    </td>
  </tr>
</table>

---

## 🎯 Current Learning Roadmap

```
2026 Path
│
├── 🐳  Docker & Containerization          ← In Progress
├── ☁️  AWS (EC2, S3, IAM, VPC)            ← In Progress
├── 🔁  CI/CD — GitHub Actions / Jenkins   ← Upcoming
├── 📡  Networking for DevOps              ← Upcoming
├── 🏗️  Terraform — Infrastructure as Code ← Planned
│
└── 🏆  AWS Cloud Practitioner Cert        ← Target: 2026
```

---

## 📊 GitHub Stats

<div align="center">

<!-- Streak Stats — most reliable, always works -->
<img src="https://streak-stats.demolab.com?user=kapilskumbhare-cyber&theme=dark&hide_border=true&background=0d1117&ring=00FF41&fire=00FF41&currStreakLabel=00FF41&sideLabels=00FF41&dates=ffffff&sideNums=ffffff&currStreakNum=00FF41" alt="GitHub Streak" width="70%"/>

<br/><br/>

<!-- Profile Summary Cards — reliable alternative to github-readme-stats -->
<img src="https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=kapilskumbhare-cyber&theme=github_dark" alt="Profile Summary" width="70%"/>

<br/>

<img src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=kapilskumbhare-cyber&theme=github_dark" alt="Repos Per Language" width="34%"/>
<img src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=kapilskumbhare-cyber&theme=github_dark" alt="Stats" width="34%"/>

</div>

---

## 🤝 Let's Connect

I'm actively looking for **entry-level DevOps / Cloud / Linux opportunities** — internships, junior roles, or mentorship.

<div align="center">

[![GitHub](https://img.shields.io/badge/GitHub-kapilskumbhare--cyber-181717?style=for-the-badge&logo=github&logoColor=white)](https://github.com/kapilskumbhare-cyber)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Kapil%20Kumbhare-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/kapilkumbhare29/)
[![Email](https://img.shields.io/badge/Gmail-kapilskumbhare%40gmail.com-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:kapilskumbhare@gmail.com)

<br/>

> 📬 *Open my repos, read the code, and reach out. I build things — you can verify that.*

</div>

---

<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:00FF41,100:000000&height=120&section=footer" width="100%"/>

```bash
$ echo "Always Learning. Always Building."
Always Learning. Always Building.
```

![Visitor Badge](https://visitor-badge.laobi.icu/badge?page_id=kapilskumbhare-cyber.kapilskumbhare-cyber)

</div>
